...
int main(int argc, char *argv[])
{
    ...
    for(int sample = 0; sample < 1; sample++){
        ...
        double p_rho = 00.034/2; // in 1/hour production of fibroblasts naturally, proliferation rate, not normalized, based on data of doubling rate from commercial use
        ...
        std::vector<int> meshResolution = {25,25};
        ...
        double x_axis = 7.5 + tol_boundary;
        double y_axis = 7.5 + tol_boundary;
        ...
        for(int nodei=0;nodei<n_node;nodei++){
            ...
            //std::cout << distance << "\n";
            if(check_ellipse<=1){ //check_ellipse<=1 distance <= x_axis/2
                // inside
        ...
        for(int elemi=0;elemi<n_elem;elemi++){
            for(int ip=0;ip<IP_size;ip++)
            {
                ...
                if(check_ellipse_ip<=1){ //check_ellipse<=1 distance <= x_axis/2
